...
export class ProductServiceProxy {
     ...
     * @return Success
     */
    getAll(productName: string | null | undefined, skipCount: number | undefined, maxResultCount: number | undefined): Observable<ProductListDtoListResultDto> {
    ...
    protected processGetAll(response: HttpResponseBase): Observable<ProductListDtoListResultDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
     ...
     * @return Success
     */
    createProduct(body: CreateProductInput | undefined): Observable<void> {
     ...
     * @return Success
     */
    getProductItems(productName: string | null | undefined, skipCount: number | undefined, maxResultCount: number | undefined): Observable<ProductListDtoPagedResultDto> {
        let url_ = this.baseUrl + "/api/services/app/Product/GetProductItems?";
        if (productName !== undefined)
            url_ += "ProductName=" + encodeURIComponent("" + productName) + "&";
        if (skipCount === null)
            throw new Error("The parameter 'skipCount' cannot be null.");
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGetProductItems(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetProductItems(<any>response_);
                } catch (e) {
                    return <Observable<ProductListDtoPagedResultDto>><any>_observableThrow(e);
                }
            } else
                return <Observable<ProductListDtoPagedResultDto>><any>_observableThrow(response_);
        }));
    }

    protected processGetProductItems(response: HttpResponseBase): Observable<ProductListDtoPagedResultDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = ProductListDtoPagedResultDto.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
        ...
        return _observableOf<ProductListDtoPagedResultDto>(<any>null);
    }
}
...
export class ProductListDtoPagedResultDto implements IProductListDtoPagedResultDto {
    totalCount: number;
    items: ProductListDto[] | undefined;

    constructor(data?: IProductListDtoPagedResultDto) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): ProductListDtoPagedResultDto {
        data = typeof data === 'object' ? data : {};
        let result = new ProductListDtoPagedResultDto();
        result.init(data);
        return result;
    }
    ...
    clone(): ProductListDtoPagedResultDto {
        const json = this.toJSON();
        let result = new ProductListDtoPagedResultDto();
        result.init(json);
        return result;
...
export interface IProductListDtoPagedResultDto {
    totalCount: number;
    items: ProductListDto[] | undefined;
}
